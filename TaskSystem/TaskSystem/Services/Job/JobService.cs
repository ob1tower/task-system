...
public class JobService : IJobService
{
    ...
    public async Task<Result<JobGetDto>> GetJob(Guid id)
    {
        ...
        if (jobs == null)
        {
            return Result.Failure<JobGetDto>($"Задача с Id {id} не найдена.");
        }
    ...
    public async Task<Result<Guid>> CreateJobs(JobCreateDto jobCreateDto)
    {
        ...
        if (projectId == null)
        {
            return Result.Failure<Guid>($"Проект с Id {jobCreateDto.ProjectId} не найден.");
        }
        ...
        if (title != null)
        {
            return Result.Failure<Guid>($"Задача с заголовком '{jobCreateDto.Title}' уже существует.");
        }
    ...
    public async Task<Result<Guid>> UpdateJobs(Guid id, JobUpdateDto jobUpdateDto)
    {
        ...
        if (projectId == null)
        {
            return Result.Failure<Guid>($"Проект с Id {jobUpdateDto.ProjectId} не найден.");
        }

        var title = await _jobRepository.SearchTitle(jobUpdateDto.Title);
        if (title != null && title.JobId != id)
        {
            return Result.Failure<Guid>($"Задача с заголовком '{jobUpdateDto.Title}' уже существует.");
        }
