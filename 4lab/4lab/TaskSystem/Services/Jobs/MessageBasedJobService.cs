...
public class MessageBasedJobService : IMessageBasedJobService
{
    private readonly IJobRepository _jobRepository;

    public MessageBasedJobService(IJobRepository jobRepository)
    {
        _jobRepository = jobRepository;
    }

    public async Task<Result<Guid>> CreateJobAsync(JobCreateDto jobCreateDto)
    {
        ...
        var job = new Models.Job(Guid.NewGuid(), jobCreateDto.Title, null,
    ...
    public async Task<Result<List<JobGetDto>>> GetAllJobsAsync(int pageNumber, int pageSize)
    {
    ...
    public async Task<Result<JobGetDto>> GetJobAsync(Guid id)
    {
    ...
    public async Task<Result<Guid>> UpdateJobAsync(Guid id, JobUpdateDto jobUpdateDto)
    {
    ...
    public async Task ProcessJobMessageAsync(JobMessageBase message)
    {
        switch (message)
        {
            case CreateJobMessage createMsg:
                _ = await CreateJobAsync(createMsg.JobCreateDto);
                break;
            case UpdateJobMessage updateMsg:
                _ = await UpdateJobAsync(updateMsg.JobId, updateMsg.JobUpdateDto);
                break;
            case DeleteJobMessage deleteMsg:
                _ = await DeleteJobAsync(deleteMsg.JobId);
                break;
            case GetJobMessage getMsg:
                _ = await GetJobAsync(getMsg.JobId);
                break;
            case GetAllJobsMessage getAllMsg:
                _ = await GetAllJobsAsync(getAllMsg.PageNumber, getAllMsg.PageSize);
                break;
        }
    }
}